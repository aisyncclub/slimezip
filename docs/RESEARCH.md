...
### 권한 지속성 — 위 단락은 오염된 측정이었다 (정정)

~~애드혹 서명이라도 권한이 리빌드를 견딘다~~ — **틀렸다.** 그 측정은 셸에서
실행 파일을 직접 돌린 것이었고, 그렇게 실행하면 TCC가 접근성 권한을 ZipBar가
아니라 **터미널 호스트 앱 기준으로** 판정한다. 호스트에 권한이 있으니 항상
true가 나왔던 것이다. 결정적 증거: 같은 순간에 GUI로 실행한 인스턴스는
`axTrusted=false`, 셸 진단은 `true`.

실제 동작: TCC는 부여 시점 서명의 지정 요구사항(designated requirement)을
기억한다. 애드혹 서명의 DR은 그 빌드의 cdhash라 리빌드마다 어긋나고, 시스템
설정 토글은 켜져 있는데 새 빌드는 남남 취급을 받는 죽은 항목이 된다.

**해결: 자체 서명 인증서 "ZipBar Dev".** DR이
`identifier "com.zipbar.ZipBar" and certificate leaf = H"4e64…"`로 고정되어
권한이 모든 리빌드를 견딘다. 죽은 항목은 `tccutil reset Accessibility
com.zipbar.ZipBar`로 제거 후 한 번 재부여.

교훈: **권한 측정은 반드시 최종 사용 경로(GUI 실행)로 해야 한다.** 셸 실행
진단은 TCC 귀속이 달라 이 계열의 측정에 쓸 수 없다.
