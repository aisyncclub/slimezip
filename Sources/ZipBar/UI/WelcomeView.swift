...
struct WelcomeView: View {
    ...
    private var identity: some View {
        HStack(alignment: .top, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text(L("메뉴바에 아이콘이 넘칠 때, 슬라임 한 마리가 나머지를 물고 있습니다."))
                    .font(.callout)
    ...
    private var versionLine: String {
        ...
        return L("버전 %@ · macOS 14 이상 · 무료 · 오픈소스", version)
    }
    ...
    private let moves: [Move] = [
        Move(number: "1", title: L("슬라임을 누릅니다"),
             detail: L("메뉴바 오른쪽의 슬라임을 클릭하면 지금 떠 있는 아이콘이 전부 나옵니다.")),
        Move(number: "2", title: L("넣기 · 꺼내기"),
             detail: L("감추고 싶은 것 옆의 버튼을 누릅니다. 여러 개를 한 번에 골라도 됩니다.")),
        Move(number: "3", title: L("적용은 한 번뿐"),
             detail: L("그 앱을 한 번만 재시작하면 자리가 정해집니다. 이후로는 클릭 즉시 됩니다.")),
    ]
    ...
    private var quickStart: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                Button(L("아이콘 목록 열기"), action: onStart)
                ...
                if !engine.capabilities.canEnumerate {
                    Label(L("손쉬운 사용 권한을 켜야 목록에 이름이 나옵니다"),
    ...
    private var creator: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 14) {
                ...
                Button(L("링크 모음")) { CreatorLinks.open(CreatorLinks.home) }
